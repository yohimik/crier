...
Render an HTML template to an image or a video, and publish it to twelve social platforms with one command.
...
- **Twelve platforms.** Instagram, Facebook, TikTok, Telegram, X, Mastodon, Discord, LinkedIn, Reddit, Slack, VK, and Threads. Publish images, video, and animated GIFs. You can also use any shell script you like as a [platform of your own](./docs/publishing/custom.md).
...
subtitle: One template, twelve platforms, one command.
```
...
CARD_SUBTITLE="One template, twelve platforms, one command." \
  crier render
```
