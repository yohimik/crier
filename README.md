...
Render an HTML template to an image or a video, and publish it to eleven social platforms with one command.
...
- **Eleven platforms.** Instagram, Facebook, TikTok, Telegram, X, Mastodon, Discord, LinkedIn, Reddit, Slack, and VK. Publish images, video, and animated GIFs. You can also use any shell script you like as a [platform of your own](./docs/publishing/custom.md).
...
subtitle: One template, eleven platforms, one command.
```
...
CARD_SUBTITLE="One template, eleven platforms, one command." \
  crier render
```
