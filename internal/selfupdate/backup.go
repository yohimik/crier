...
// BackupVersion is the version the kept binary reports, or an error when there
// is no backup or it does not run.
//
// It is what --check answers with before a rollback, and what a rollback
// verifies before committing to one.
func BackupVersion(ctx context.Context, exe string) (string, error) {
	...
	defer cancel()
	out, err := exec.CommandContext(ctx, backup, "--version").CombinedOutput()
	if err != nil {
		return "", fmt.Errorf("self-update: the backup at %s does not run: %w", backup, err)
	}
	return ParseVersionOutput(string(out)), nil
}

// ParseVersionOutput picks the version out of what `crier --version` prints:
// "crier <version> (commit ..., built ..., go..., os/arch)".
//
// An output that does not read that way answers "", which every caller treats
// as "cannot tell", never as a match.
func ParseVersionOutput(out string) string {
	for _, line := range strings.Split(out, "\n") {
		rest, ok := strings.CutPrefix(strings.TrimSpace(line), "crier ")
		if !ok {
			continue
		}
...
// corrupt file must not be discovered after it is the only crier left.
func Rollback(ctx context.Context, exe string) (from, to string, err error) {
	if exe == "" {
		if exe, err = Executable(); err != nil {
			return "", "", fmt.Errorf("self-update: locating the running binary: %w", err)
		}
	}
