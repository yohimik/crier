...
// new one in works identically on every platform crier ships for. Neither
...
func Replace(exe, incoming string) (backup string, err error) {
	...
	if err := os.Remove(backup); err != nil && !os.IsNotExist(err) {
		return "", fmt.Errorf("self-update: removing the previous backup %s: %w", backup, err)
	}
	if _, err := os.Lstat(exe); os.IsNotExist(err) {
		// Nothing to step aside. The single rename below is then the whole
		// install, and it is atomic, so the path holds either the previous
		// nothing or the finished file, never a half-written one.
		if err := os.Rename(incoming, exe); err != nil {
			return "", fmt.Errorf("self-update: installing %s: %w", exe, err)
		}
		return "", nil
	}
	if err := os.Rename(exe, backup); err != nil {
		return "", fmt.Errorf("self-update: moving %s aside: %w", exe, err)
	}
	if err := os.Rename(incoming, exe); err != nil {
		...
		if back := os.Rename(backup, exe); back != nil {
			return "", fmt.Errorf("self-update: installing %s failed (%w) and restoring %s failed too (%v): "+
				"the previous binary is at %s", exe, err, exe, back, backup)
		}
		return "", fmt.Errorf("self-update: installing %s: %w", exe, err)
	}
	// The backup's clock starts now. Without this it would carry the mtime the
	// outgoing binary was built or downloaded with, and PruneBackup would read
	// that as an age it does not have.
	touch(backup)
	return backup, nil
}
...
// version is ever lost, which is what lets a second one return.
func Restore(exe string) error {
	...
	if err != nil {
		return fmt.Errorf("self-update: %s: %w (%v); re-run with the rights to replace %s",
	...
	if err := os.Remove(parkedName); err != nil {
		return fmt.Errorf("self-update: %s: %w", parkedName, err)
	}
	...
	if err := os.Rename(backup, exe); err != nil {
		if back := os.Rename(parkedName, exe); back != nil {
			return fmt.Errorf("self-update: restoring %s failed (%w) and putting the current binary "+
				"back failed too (%v): it is at %s", exe, err, back, parkedName)
		}
		return fmt.Errorf("self-update: restoring %s: %w", exe, err)
	}
	...
	if err := os.Rename(parkedName, backup); err != nil {
		...
		return fmt.Errorf("self-update: restored %s, but %w: %v", exe, ErrBackupNotKept, err)
	}
	touch(backup)
	return nil
}
