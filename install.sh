...
#   wget -qO- https://raw.githubusercontent.com/yohimik/crier/main/install.sh | sh
#
# One script for every Unix: the release binaries are static (CGO_ENABLED=0), so
# there is nothing to compile and nothing to link against, and the only tools
# this needs are the ones every base image already has — a POSIX shell and
# either curl or wget.
#
# Output contract: the resolved version goes to stdout, alone, and everything
# written for a human goes to stderr. `version=$(sh install.sh)` is therefore the
# whole of what a caller needs to report what it installed.
set -eu
...
TAG_PREFIX="v"
API_URL="${CRIER_API_URL:-https://api.github.com}"
DOWNLOAD_URL="${CRIER_DOWNLOAD_URL:-https://github.com}"

VERSION="${CRIER_VERSION:-}"
BIN_DIR="${CRIER_BIN_DIR:-}"
TOKEN="${GITHUB_TOKEN:-}"
OS=""
ARCH=""
...
usage() {
	cat >&2 <<'EOF'
Install the crier CLI.

Usage: install.sh [options]

Options:
  --version <v>    Version or tag to install: 1.2.3 or v1.2.3.
...
Environment: CRIER_VERSION, CRIER_BIN_DIR and GITHUB_TOKEN are read as the
...
if [ -z "$VERSION" ]; then
	...
	# Prereleases drop out on the "-" test, which needs no JSON structure: a
	# stable version has no hyphen in it.
	TAGS=""
	PAGE=1
	while [ "$PAGE" -le 3 ]; do
...
case "$VERSION" in
[0-9]*.[0-9]*.[0-9]*) ;;
*) die "not a version: $VERSION (expected 1.2.3 or v1.2.3)" ;;
esac
...
# GitHub reports a "digest" per asset. Fetching the release by tag also turns
# "no such version" into a clean failure here rather than a 404 on the download.
...
# one-off export and the line that makes it permanent, aimed at the profile the
# user's shell actually reads. A directory already on PATH can still lose to an
# older crier installed somewhere earlier, which looks exactly like the new
# version failing to install — so that shadowing is said out loud.
case "${SHELL:-}" in
