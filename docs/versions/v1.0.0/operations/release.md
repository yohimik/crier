...
crier publishes one alias tag, `v1`, declared with `channels: [stable]` so a candidate can never drag it forward: it moves only when a stable release does, which is what lets `uses: yohimik/crier@v1` mean the stable line.
...
`commitErrors` is set to `error`: a commit dispat cannot parse refuses the release rather than silently contributing nothing. The setting only became available with the first tag. The seven commits that predate crier's conventional-commit convention are unparseable, and until `v1.0.0-rc.0` existed they sat inside every release window; now they sit outside it.
...
Every release posts a feed card and a story to Instagram, and one post to LinkedIn that carries the whole release: the reel. A LinkedIn post takes one video or many images, never both — `content.multiImage` is documented images-only — so the reel puts the pages inside the video: the cover holds under the fanfare, then each changelog page follows in reading order, all in the anthem's sixteen seconds. When LinkedIn refuses the video (uploading one needs `w_member_social` on the app, and older tokens minted before the product was added keep answering 403), the pass falls back to the whole card as one multi-image album, cover first, so the release reaches LinkedIn with whatever the token allows. The commentary is LinkedIn's own, written around the fact that the post is generated: the changelog comes from the commits, and the render and the publish both happen inside the GitHub Actions run. LinkedIn takes the bytes directly, so that pass needs no tunnel. The `crier` tool renders these from the binary that the release just built. It runs in the `announce` stage of `dispat`. This stage only ever warns. The `announce/announce.sh` script exits with 0 for any reason it decides not to post. **A missing secret can never turn a good release red.**
...
All three are set in this repository. Without them, a release logs what is missing and skips the announcement.
