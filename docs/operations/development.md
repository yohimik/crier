...
In that mode, nothing is built and no coverage is collected. A released binary is not instrumented. `^TestSmoke` is the subset the release runs: a render, the configuration precedence, the twelve-platform fan-out, and the version stamp. See [releasing](./release.md).
