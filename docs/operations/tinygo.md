...
With the three in place, the fork links crier for `linux/amd64`, `linux/arm64` and `darwin/arm64`. That is where the good news ends.

**A fork-built crier renders nothing.** On linux it starts, reports its version and platform, and panics on its first template:

```
panic: unimplemented: (reflect.Type).NumOut()
```

`text/template` inspects every function it is handed — the built-ins included — through `reflect.Type.NumOut`, `NumIn`, `In` and `Out`, and calls them through `reflect.Value.Call`. TinyGo's reflect implements none of those, in the fork exactly as upstream: each is a stub that panics with `unimplemented`. Templates are how crier lays out a card, writes a caption and reads a data document, so the panic is not one feature missing but the program's core. It is a toolchain limit, not a library one — there is no build tag to pass and no shim to write, because the stubs are definitions, not declarations — and it is the reason every release binary stays gc's.

**A fork-built darwin binary crashes before `main`.** `golang.org/x/sys/cpu` probes Apple silicon's features through `sysctl`, reached by a libc trampoline the gc linker fills in from a `//go:cgo_import_dynamic` directive. TinyGo's linker fills in nothing, so the call lands on address zero inside `runtime.initAll`, before a line of crier runs. `darwin-run.log` records it, and every fork row of the darwin matrix records the same.

**The version line names the fork.** `runtime.Version()` under TinyGo is the toolchain's own version, so `crier --version` reports `0.43.0-net.1` where a gc binary reports `go1.26.x`. The e2e smoke subset holds the version line to `go1.`, and would refuse a fork-built binary on that alone.

**Compiling it takes patience.** TinyGo runs package initialisers at compile time, and minio-go's `regexp.MustCompile` calls take that pass minutes. Its default budget is three, which a builder sharing its cores overran once, so the spike raises it with `-interp-timeout=15m`. A fork build of crier is around seven minutes on an M-series Mac, and fourteen inside the Docker VM.

What a fork-built crier *can* do, the spike still asks, because it is what will matter the day the reflect gap closes. `tinygo-spike-e2e` runs the black-box suite against the linux/arm64 fork build, and the answer at 0.43.0-net.1 is 47 tests passing, 89 failing and 3 skipped. Of the 89 failures, 82 are the panic above, one per test that renders or writes a caption. The 47 that pass are the tests that never reach a template: usage and configuration errors, `crier ping` against every platform's fake, `crier platforms`, `crier init`, the publish-only and encode-only flows over files that already exist — and all eight self-update tests, which download, verify, swap in and roll back a binary over the fork's own `net/http`. So the fork's network, TLS, process spawning and file handling carry crier fine; it is the toolchain's reflect that does not. `tinygo-spike-selfupdate` asks the same of the update path with real TLS in the way, below.
...
One thing, in one of two places.

On the toolchain's side, TinyGo's reflect would have to implement function introspection and invocation — `Type.NumIn`, `NumOut`, `In`, `Out`, `IsVariadic` and `Value.Call`, the six stubs `text/template` trips on — upstream or in the fork.

On crier's side, the templates could stop needing them. `text/template/parse` is reflect-free and compiles under TinyGo as it is; only the executor in `text/template` (and the contextual escaper `html/template` builds on it) reaches for reflect, to walk fields of arbitrary structs and to call arbitrary functions. crier's data is never an arbitrary struct — a data document is YAML or JSON, so maps, lists, strings, numbers and booleans — and its function set is its own: `upper`, `lower`, `title`, `trim`, `join`, `repeat`, `now`, `date`, `default`, `dict` and the seeded random helpers. An executor over the standard parse tree that walks generic values and dispatches functions through a typed table needs no reflect at all, and the layouts, overlays and captions in the examples use `if`, `else`, `range`, `block`, `define`, `template` and pipelines, nothing the parse tree does not already hold. It would be one executor for every build, not a TinyGo-only one, so the gc binaries would change behaviour with it: `html/template`'s context-sensitive escaping would become whatever escaping the executor does, and every template test would be held to the new implementation. It is a real project rather than a shim, and it is the first door only — the black-box suite against a fork build is what says whether the YAML decoder, minio-go's XML, the layout engine and the rasteriser run under TinyGo's reflect and runtime once a template does. The spike's `e2e` stage is where that answer would appear.

Everything else is already here. The install manifest fetches the fork and lays cookiejar over it; the shim package links the assembly-bearing dependencies; the spike builds, updates and tests the result; and dispat's own root Dockerfile shows what the release stages look like once a fork-built binary is worth shipping: two more linux binaries under names of their own, additive to the six the [asset contract](./install.md#the-asset-names-are-a-contract) names, validated and put through the black-box suite before upload.

Re-asking the fork's question is bumping one version number in [`scripts/install-tools.sh`](../../scripts/install-tools.sh) and running one command. Re-asking upstream's is bumping `TINYGO_VERSION` at the top of `Dockerfile.tinygo`.
