...
**The fork's `net` dials but does not listen.** Its host netdev has `Bind`, `Listen` and `Accept`, but `net.ListenConfig.Listen` above them is still the stub upstream ships, so nothing can serve. crier's stage server — the default way an image reaches Instagram's fetcher, through a tunnel — listens, and every test that stages through it fails under the fork at that line. The publish paths that upload bytes directly, and staging through S3 or a URL somebody else serves, do not listen and pass. That is the one gap left between the fork and the whole suite, and it is the fork's to close.
...
On crier's side, the templates could stop needing them — and since 1.0.1 they do. `text/template/parse` is reflect-free and compiles under TinyGo as it is; only the executor in `text/template` (and the contextual escaper `html/template` builds on it) reached for reflect, to walk fields of arbitrary structs and to call arbitrary functions. crier's data is never an arbitrary struct — a data document is YAML or JSON, so maps, lists, strings, numbers and booleans — and its function set is its own. [`internal/template/exec`](../../internal/template/exec/) is an executor over the standard parse tree that walks generic values and dispatches functions through a typed table, with no reflect in it, held to text/template's behaviour by tests that run the same templates through both and require the same output. It is one executor for every build, not a TinyGo-only one, so the gc binaries changed with it in the ways [the template documentation](../templates/README.md#how-values-are-escaped) lists: one escaping rule where `html/template` had a context machine, comments kept, methods limited to times. That was the first door only; the black-box suite against a fork build is what says whether the YAML decoder, minio-go's XML, the layout engine and the rasteriser run under TinyGo's reflect and runtime once a template does, and the spike's `e2e` stage is where that answer appears.
