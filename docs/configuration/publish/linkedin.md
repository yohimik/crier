...
| `publish.linkedin.lead-video`<br>`CRIER_PUBLISH_LINKEDIN_LEAD_VIDEO`<br>`--publish-linkedin-lead-video` | string, path | — | not available: a linkedin post is pictures or a video and never both, so a value here is refused rather than ignored |
