...
| `publish.mastodon.lead-video`<br>`CRIER_PUBLISH_MASTODON_LEAD_VIDEO`<br>`--publish-mastodon-lead-video` | string, path | — | not available: a mastodon post is pictures or a video and never both, so a value here is refused rather than ignored |
